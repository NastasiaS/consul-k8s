...
service_prefix "" {
   policy = "read"
	...
	aclPolicy, _, err := consulClient.ACL().PolicyCreate(&agentPolicy, &api.WriteOptions{Token: bootstrapToken.SecretID})
	if err != nil {
	...
	token := api.ACLToken{
		...
		Policies:    []*api.ACLTokenPolicyLink{&api.ACLTokenPolicyLink{Name: aclPolicy.Name}},
	}
